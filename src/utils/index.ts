...
export function hideEmail(email: string) {
  ...
  const maskedUsername =
    username.substring(0, 5) + "*".repeat(username.length - 5);
...
export const countdownInSeconds = (fromTime: string, toTime: string) => {
  const fromDateTime = DateTime.fromISO(fromTime);
  const toDateTime = DateTime.fromISO(toTime);

  const diffInSeconds = Math.floor(toDateTime.diff(fromDateTime).as("seconds"));
